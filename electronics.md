...
| Item | Spec | Status |
|---|---|---|
| Prototype board | **Toradex Verdin Development Board** — takes any Verdin module (iMX8M Plus now, **iMX95 later** — pin-compatible), so it carries straight to the bespoke carrier; full BSP + display/camera/connectivity bring-up. (NXP i.MX 8M Plus EVK is the generic fallback.) | `[BUY]` eval |
| Production compute | **Decided: Toradex Verdin iMX8M Plus** on a bespoke carrier. Chosen for best-in-class carrier-design docs + BSP (Torizon + Zephyr on the M7), dual CAN-FD on the connector, −40/+85, **guaranteed availability to 2036+**, and — decisively — the Verdin form factor is **pin-compatible to Verdin iMX95**, a drop-in upgrade path to the up-alternative below. Origin **Switzerland** (Europe/EFTA — accepted under the sourcing rule). In-EU alternatives if strict membership needed: PHYTEC phyCORE (DE), Engicam i.Core (IT). **Do not run the bare EVK in the bike.** | `[LOCKED]` / `[BUY]` + `[BESPOKE]` carrier |
...
| Vehicle bus | **Dual CAN-FD controllers** (native on the 8M Plus) — but the ECU link runs **classic CAN** (the microRusEFI's F7 has no FDCAN); FD controllers speak classic natively. Data dictionary assumes ≤8-byte frames | Feeds the M7 safety cluster |
...
2. **Modern engine, custom brain — that's the hard part** — the CP3 is new and Euro 5 (good for reliability, parts and emissions), but it's ride-by-wire + CAN + immobiliser. Running it standalone means re-creating throttle control, handshakes and emissions yourself, and buying a whole new donor bike to get the engine. Prototype on the XSR900 GP mule first (see the ECU-on-mule dev runbook).
