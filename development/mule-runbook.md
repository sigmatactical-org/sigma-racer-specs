...
> **Golden rule:** every failure that can hurt an engine happens here — never on the build engine. **The mule is a NEW XSR900 GP (on order), not a disposable lump** — so the phase gates are the *only* safety net: nothing advances until its **gate** passes, ignition stays conservative until knock is characterized, and no experiment runs that a gate hasn't earned. Bonus: the GP is also `chassis.md`'s locked stem-geometry reference — measure it while it's in hand.
...
| Item | Note | Status |
|---|---|---|
| Mule: **new XSR900 GP** | **On order.** Gen-3 CP3, Euro 5+ — gen-identical to production MT-09 donors, so characterization carries 1:1. Complete bike, kept intact/rideable; prefer non-invasive work first (warranty note, Phase 0) | `[LOCKED]` on order |
...
2. **The mule is new and premium — the gates replace disposability as the safety net.** Every mistake still belongs here rather than on a production unit, but there is no cheap-lump write-off anymore: run conservative maps until knock is characterized, and never skip a gate. If the GP comes through un-abused, it graduates — keeper bike, rolling testbed, or donor #1. The build engine still only ever runs mule-proven firmware.
