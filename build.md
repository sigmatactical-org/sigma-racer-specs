...
*Companion to `README.md`. The whole-build rollup the domain docs don't carry: mass target, long-lead procurement, build phases and the test/sign-off plan (cost/economics maintained internally — §2). Domain detail lives in `engine.md`, `chassis.md`, `electronics.md`, `electrical.md`, `bodywork.md`; homologation in `emissions_certification.md`.*
...
*Maintained internally — per-unit COGS, NRE, pricing, breakeven and the business-model findings are working numbers, not part of the published spec. The engineering consequences that domain docs rely on stand on their own: the donor-harvest **production supply** model and its validation status live in `engine.md` §1; DfM and volume constraints are tracked as build caveats.*
